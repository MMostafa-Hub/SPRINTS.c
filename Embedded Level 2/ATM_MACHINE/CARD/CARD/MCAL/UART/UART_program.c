...
**********************************************************************************************************************************/
void MUSART_u8UsartReceiveString(u8 *copy_u8PtrStr , u16 *copy_u16ArraySize )
{
	/* local Index */
	u16 local_u16Indx = 0 ;
	/* Receive the First char And Store it in the incoming Array */
	copy_u8PtrStr[local_u16Indx] = MUSART_u8UsartReceiveChar() ;
	/* Stop When the User press Enter */
	while(copy_u8PtrStr[local_u16Indx]  != ENTER_ASCII)
	{
		/* increment the index */	
		local_u16Indx++;
	...
	*copy_u16ArraySize  = local_u16Indx ;
	/* Clear the last Char */
